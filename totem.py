...
#arduino = serial.Serial(port='COM0', baudrate=115200, timeout=0.1)
...
while True:
    try:
        if not time.time() - static_frame_time < 3 and not static_frame:
            ret, frame = cap.read()
            if not ret:
                pass
            ...
            if locs:
                ...
                # opening the gate 
                if label == "Com Mascara" and data == 0: # data == 0 means that the gate can be opened
                    arduino.write(bytes(1, 'utf-8'))
                    static_frame = True
                    static_frame_time = time.time()
                elif label == "Sem Mascara":
                    static_frame = True
                    static_frame_time = time.time()
