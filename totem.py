...
def update_screen():
    global t_frame, x1, x2, y1, y2, cls_pred, conf, stop
    while stop:
        # Black image
...
if __name__ == "__main__":
    ...
    x = threading.Thread(target=update_screen, args=())
    x.start()
    ...
    while True:
        ...
        if int(cls_pred) == 0:
            # WITH_MASK
            org_frame = cv2.rectangle(t_frame, (x1, y1), (x2, y2), (0, 255, 0), 2).copy()
        else:
            # WITHOUT_MASK
            org_frame = cv2.rectangle(t_frame, (x1, y1), (x2, y2), (0, 0, 255), 2).copy()
        cv2.putText(org_frame, classes[int(cls_pred)] + ": %.2f" % conf, (x1, y1 + t_size[1]), cv2.FONT_HERSHEY_PLAIN,
                    1, [225, 255, 255], 2)
        org_frame = cv2.UMat.get(t_frame)
